...
public interface MoviesController {
    ...
    @GetMapping("/{movieId}")
    HttpEntityResponse<MovieDTO> findMovieByMovieId(@PathVariable("movieId")
    ...
    HttpEntityResponse<List<MovieDTO>> findMoviesByMoviesTitleContaining(@PathVariable(name = "title") String movieTitle,
    ...
    HttpEntityResponse<List<MovieDTO>> findTopRatedMovies(@RequestParam(name = "page_size")
                                                   @Min(value = 1, message = "Parameter 'page_size' cannot be less than 1")
                                                   @Max(value = 100, message = "Parameter 'page_size' cannot be greater than 100")
                                                   int pageSize,
                                                   @RequestParam(name = "page")
    ...
    HttpEntityResponse<List<MovieDTO>> findTopMoviesByGenreName(@PathVariable(name = "genre") String genre,
                                                         @RequestParam(name = "page_size")
                                                         @Min(value = 1, message = "Parameter 'page_size' cannot be less than 1")
                                                         @Max(value = 100, message = "Parameter 'page_size' cannot be greater than 100")
                                                         int pageSize,
    ...
    HttpEntityResponse<List<MovieDTO>> findTopMoviesByGenreId(@PathVariable(name = "id")
    ...
    HttpEntityResponse<List<PersonDTO>> findDirectorsByMovieId(@PathVariable("movieId")
                                                        @Min(value = 1, message = "Movie id cannot be less than 1")
                                                        long movieId);

    @GetMapping("/{movieId}/writers")
    HttpEntityResponse<List<PersonDTO>> findWritersByMovieId(@PathVariable("movieId")
                                                      @Min(value = 1, message = "Movie id cannot be less than 1")
                                                      long movieId);

    @GetMapping("/{movieId}/actors")
    HttpEntityResponse<List<PersonDTO>> findActorsByMovieId(@PathVariable("movieId")
                                                     @Min(value = 1, message = "Movie id cannot be less than 1")
                                                     long movieId);
}
